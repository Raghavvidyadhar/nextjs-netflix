...
export default function MediaItem({
    ...
    const { setShowDetailsPopup,
        setCurrentMediaInfoIdAndType, loggedInAccount, setFavorites, similarMedias, searchResults, setSearchResults, setSimilarMedias, setMediaData, mediaData } = useContext(GlobalContext)
    ...
    async function handleRemoveFavorites(item) {
     const res = await  fetch(`/api/favorites/remove-favorites?id=${item._id}` , {
        method : 'DELETE'
     })
     const data  = await res.json()
    ...
    return (
        ...
        >
            ...
            <div className="relative  cardWrapper h-28 min-w-[180] cursor-pointer md:h-36 md:w-[260px] transform transition  duration-500 hover:scale-110 hover:z-[999]" >
                <Image
                ...
                />
                <div className="space-x-3 hidden absolute p-2 bottom-2 buttonWrapper ">
                    <button
                        onClick={media?.addedToFavorites ? listView? () => handleRemoveFavorites(media) :null 
                            : () => handleAddToFavorites(media)}
                        className={`${media?.addedToFavorites && ! listView && 'cursor-not-allowed'}cursor-pointer border flex p-2 items-center gap-x-2 rounded-full  text-sm font-semibold transition hover:opacity-90 border-white   bg-black opacity-75 text-black`}
